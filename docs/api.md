...
are accepted. Provision matching browser binaries in the execution environment. This is one runtime declaration,
...
execute in the supplied environment and use its network policy.
...
Every target launches Chromium locally. Supply `PLAYWRIGHT_BROWSERS_PATH` as an
inherited absolute path or a runfiles-relative target `env` value with browser
artifacts in `data`. Provision matching browsers and system dependencies before
testing; the runner does not install them.

For VRT, the caller runs the full Bazel suite and `.update` inside the same pinned
Linux amd64 OCI image. Image selection and network policy belong to the caller.
`playwright_runtime.image`, `PLAYWRIGHT_IMAGE`, and `playwright_images` are removed.
Nonempty `network_origins` / `network_origins_env` and consumer `use.connectOptions`
are rejected. See [setup and consumer migration](host-browsers.md).
