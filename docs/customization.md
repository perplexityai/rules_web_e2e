...
staging, environment isolation, local browser configuration, artifacts, and teardown.
...
flowchart LR
  ...
  Runtime --> Browser[Provisioned local browser]
  Browser --> Server
...
Both use the same screenshot comparison and update contract. External services depend on caller-owned networking; prefer declared fixture responses.
